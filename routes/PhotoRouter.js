...
photoRoute.get("/", async (request, response) => {
  const photos = await Photos.find({}).select("_id user_id comments file_name date_time");
...
photoRoute.get("/:id", async (req, res) => {
  try {
    ...
    const photos = await Photos.find({ user_id: userId }).select("_id user_id comments file_name date_time").populate({
...
photoRoute.post("/commentsOfPhoto/:photo_id", checkAuth, async (req, res) => {
  try {
    ...
    const resPhoto = await Photos.findById(photoId).select("comments").populate({
      path: "comments.user_id",
      select: "_id first_name last_name",
    });
    const newComment = resPhoto.comments.find((c) => c.comment === comment && c.user_id._id.toString() === userId.toString());
    res.status(200).json({
...
photoRoute.post("/new", checkAuth, async (req, res) => {
  try {
    ...
    if (err.code === "ENOENT") {
      ...
      res.status(500).json({
        message: "Server error: Image upload directory not found or created.",
        error: err.message,
      });
    } else {
...
photoRoute.delete("/:photoId", checkAuth, async (req, res) => {
  try {
    // check photo.user_id khop voi user._id
